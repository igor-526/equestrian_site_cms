...
from core.protocols.repositories.horse_service_repository import HorseServiceRepositoryProtocol
...
class HorseServiceService:
    ...
    async def create(self, data: HorseServiceCreateDto) -> HorseService:
        """Создать новую услугу."""
        horse_service_data = data.model_dump(exclude_none=True)
        
        # Проверяем уникальность name
        existing = await self.horse_service_repository.find_by_name(horse_service_data["name"])
        if existing is not None:
            raise ClientError(f"Услуга с названием '{horse_service_data['name']}' уже существует")
        ...
        horse_service_data["slug"] = await self._ensure_unique_slug(horse_service_data["slug"])
        
        # Устанавливаем page_data по умолчанию, если не задан
        if "page_data" not in horse_service_data or horse_service_data["page_data"] is None:
            horse_service_data["page_data"] = "<div></div>"
        
        horse_service = HorseService(**horse_service_data)
        return await self.horse_service_repository.create(horse_service)

    async def update(self, slug_or_id: str, data: HorseServiceUpdateDto) -> HorseService:
        """Обновить услугу."""
        ...
        if "name" in update_data:
            existing = await self.horse_service_repository.find_by_name(update_data["name"])
            if existing is not None and existing.id != horse_service.id:
                raise ClientError(f"Услуга с названием '{update_data['name']}' уже существует")
        ...
        if "name" in update_data and "slug" not in update_data:
            ...
            update_data["slug"] = await self._ensure_unique_slug(new_slug, exclude_id=horse_service.id)
    ...
    async def get_by_slug_or_id(self, slug_or_id: str) -> HorseService | None:
