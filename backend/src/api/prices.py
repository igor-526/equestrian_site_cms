...
from core.protocols.repositories.price_repository import PriceGroupRepositoryProtocol, PriceRepositoryProtocol
from core.protocols.repositories.photo_repository import PhotoRepositoryProtocol
from depends.repositories import get_price_group_repository, get_price_repository, get_photo_repository
from depends.services import get_price_service, get_price_group_service
...
async def _enrich_price_with_relations(
    ...
    if photo_ids:
        ...
        photos_relations_sorted = sorted(
            photos_relations,
            key=lambda rel: (not rel.is_main, rel.photo_id)
        )
        ...
        for rel in photos_relations_sorted:
            if rel.photo_id in photos_dict:
                ...
                photo_url = f"{protocol}://{settings.cms_backend_domain}/media/{photo.path}"
...
async def get_prices(
    ...
    name: str | list[str] | None = Query(None, description="Фильтр по названию (вхождение или список)"),
    description: str | None = Query(None, description="Фильтр по описанию (вхождение)"),
    groups: str | list[str] | None = Query(None, description="Фильтр по группам (полное совпадение)"),
    sort: list[Literal["name", "-name"]] | None = Query(None, description="Сортировка"),
    limit: int | None = Query(None, description="Лимит"),
    offset: int | None = Query(None, description="Смещение"),
